...
def create_analysis_prompt(grid_data, user_info, is_paid):
    ...
    prompt = f"""As a Feng Shui expert, analyze this bedroom layout based on the Bagua grid (3x3):
...
def parse_response(text, is_paid):
    ...
    for line in lines:
        if "POSITIVE ASPECTS" in line.upper():
            ...
            continue
        elif "NEGATIVE ASPECTS" in line.upper():
    ...
    if not is_paid:
        ...
        if first_issue:
            limited_content = "\n".join(first_issue)
            sections["negative_aspects"]["content"] = limited_content + "\n\n[Additional issues analysis available with Premium Report]"
        else:
            sections["negative_aspects"]["content"] = "[Analysis available with Premium Report]"
        
        # 限制改进建议
        sections["recommendations"]["content"] = "[Detailed recommendations available with Premium Report]"
